...
/**
 * Type annotation (as) and type conversion (to) evaluation.
 *
 * Extracted from Evaluator class — type checking, annotation,
 * and conversion logic as free functions receiving an EvalContext.
 */
...
export function evalTypeAnnotation(
  ...
  if (node.type.isTuple && node.type.tupleElements && val instanceof UzonTuple) {
    return annotateTupleType(ctx, val, node.type.tupleElements, scope, node as AstNode);
  }
  ...
  // Cross-category type validation (§6.1)
...
function annotateListType(
  ...
  if (/^[iu]\d+$/.test(elemTypeName)) {
    for (let i = 0; i < val.length; i++) {
      if (val[i] !== null && typeof val[i] === "bigint") {
        validateIntegerType(val[i] as bigint, elemTypeName, node);
      } else if (val[i] !== null && typeof val[i] !== "bigint") {
        throw new UzonTypeError(
          `List element ${i} is ${typeTag(val[i])} but expected integer (${elemTypeName})`,
          node.line, node.col,
        );
      }
    }
    ...
    for (let i = 0; i < val.length; i++) {
      if (val[i] !== null && typeof val[i] === "number") {
        validateFloatType(val[i] as number, elemTypeName, node);
      } else if (val[i] !== null && typeof val[i] !== "number") {
        throw new UzonTypeError(
          `List element ${i} is ${typeTag(val[i])} but expected float (${elemTypeName})`,
          node.line, node.col,
        );
      }
    }
    ...
    for (let i = 0; i < val.length; i++) {
      if (val[i] !== null && typeof val[i] !== "boolean") {
        throw new UzonTypeError(
          `List element ${i} is ${typeTag(val[i])} but expected bool`,
          node.line, node.col,
        );
      }
    }
    ...
    for (let i = 0; i < val.length; i++) {
      if (val[i] !== null && typeof val[i] !== "string") {
        throw new UzonTypeError(
          `List element ${i} is ${typeTag(val[i])} but expected string`,
          node.line, node.col,
        );
      }
    }
  ...
  for (let i = 0; i < expectedTypes.length; i++) {
    ...
    if (/^[iu]\d+$/.test(elemType)) {
      if (typeof elem !== "bigint") {
        throw new UzonTypeError(`Tuple element ${i} is ${typeTag(elem)} but expected ${elemType}`, node.line, node.col);
      }
      ...
      throw new UzonTypeError(`Tuple element ${i} is ${typeTag(elem)} but expected bool`, node.line, node.col);
    } else if (elemType === "string" && typeof elem !== "string") {
      throw new UzonTypeError(`Tuple element ${i} is ${typeTag(elem)} but expected string`, node.line, node.col);
    } else if (!/^[iuf]\d+$/.test(elemType) && elemType !== "bool" && elemType !== "string") {
      const elemTypeDef = scope.getType(expectedTypes[i].path);
      if (elemTypeDef) {
        if (elemTypeDef.kind === "enum" && (!(elem instanceof UzonEnum) || (elem.typeName && elem.typeName !== elemTypeDef.name))) {
          throw new UzonTypeError(`Tuple element ${i} is ${typeTag(elem)} but expected enum '${elemType}'`, node.line, node.col);
        }
        if (elemTypeDef.kind === "struct" && (typeof elem !== "object" || Array.isArray(elem)
          ...
          throw new UzonTypeError(`Tuple element ${i} is ${typeTag(elem)} but expected struct '${elemType}'`, node.line, node.col);
        }
      }
    }
  }
...
export function evalConversion(
  ...
  if (typeof val === "boolean") {
    ...
    throw new UzonTypeError(`Cannot convert bool to '${typeName}' — only bool to string or bool is permitted`, node.line, node.col);
  }
  ...
  if (typeof val === "string") {
    ...
    const converted = convertStringToNumeric(val, typeName, node as AstNode);
    if (/^[iuf]\d+$/.test(typeName)) ctx.numericType = typeName;
    return converted;
  }
...
function convertStringToNumeric(val: string, typeName: string, node: AstNode): UzonValue {
  ...
  if (typeName.startsWith("i") || typeName.startsWith("u")) {
    ...
    validateIntegerType(n, typeName, node);
    return n;
  }
